...
trait Tables {
  ...
  class Morpheme(_tableTag: Tag) extends Table[MorphemeRow](_tableTag, "morpheme") {
    ...
    /** Foreign key referencing Word (database name word_monopheme_fk) */
    lazy val wordFk = foreignKey("word_monopheme_fk", wordId, Word)(r => r.id, onUpdate=ForeignKeyAction.Cascade, onDelete=ForeignKeyAction.Cascade)
  }
